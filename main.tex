...
dissatisfaction with the stabilization~\cite{Hanakam2023}. \todo{Maybe this
paragraph can be a bit shorter? Of course important to mention previous
research, but all this info is not 100\% necessary for understanding our paper.
(I've been working on this paragraph for over a year for a few different
possible publications about the balance assist bike, I'd like to keep it in.
This is an attempt to set the record straight on where this entire idea of
doing "balance assist" derives from.-- JKM)}
...
The magnitude of the perturbation is determined using an adaptive staircase
procedure.  \todo{Cite to correct paper from thesis} The adaptive staircase
takes the nominal fall threshold as input and randomly chooses one of five
possible forces that are centred around the nominal fall threshold with
10~\si{\newton} intervals. If the perturbation results in a fall, the nominal
fall threshold is decreased with 10~\si{\newton}. If the perturbation does not
results in a fall, the nominal fall threshold is increased with
10~\si{\newton}. This adaptive staircase procedure ensures that approximately
50\% of the perturbations applied to the rider result in a fall.
%
\begin{figure}
  \centering
...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    \(b\) & boolean & balance assist control on or off \\
