...
physical acutator magnitude and bandwidth limits.  \citet{Schwab2008}
...
between the head tube and steer tube, Figure~\ref{fig:balance-assist-bicycle}.
A custom motor controller converts the commanded torque to applied torque. We
...
\begin{figure}
  ...
  \subcaptionbox{
    ...
    % TODO : This label causes a compile error. I think this is where it is
    % supposed to go.
...
on the balance assist controller indicates if the assistance is off \(k=0\) or
on at two different levels: low \(k=8\) or high \(k=10\). A recovery from the
...
\begin{table}
  ...
  \begin{tabular}{llll}
    ...
    Bicycle Speed & \(v\) & \si{\meters\per\second} & wheel encoder \\
    ...
    Roll Angle & \(\phi\) &  \si{\degree} & Kalman estimate~\cite{Gabriel2023} \\
