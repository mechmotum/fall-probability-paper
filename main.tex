...
and reported injuries. At low speeds (less than 15 km/h or so) bicycles
themselves are not self-stable and become more difficult to control for the
rider. Low speed crashes may be reduced if the bicycle was self stable at these
speeds. Since at least the 1980s it known that applying a steering torque
...
\Citet{vanZytveld1975} seems to be the first to attempt to robotically
...
predicted. In his control model, he recognized that feedback of the vehicle
roll angle and angular rate was essential to stabilize the vehicle. It was not
until the mid 1980s that someone successfully demonstrated an automatically
balanced motorcycle~\cite{Ruijs1986a} and they did so with a steer motor. Ruijs
and Pacejka clearly showed that steer torque driven by roll angle feedback
...
show how the gains need to change with respect to vehicle speed. Roll motion
feedback is the simplest controller that can stabilize a single track vehicle
above a minimum speed when you are not concerned with wobble destablization.
But Ruijs and Pacejka's work was not particularly concerned with low speed
...
stability reduces falling when ridden.  Most of these robotic bicycles and
motorcycles designers did not intend for a human rider to also control the
...
applied steer torque act on the steer in parallel. The effect of this automatic
control gives the ability to change the dynamics, to a degree, of the human
controlled plant.
...
self-stablity. It is ideal for showing the effects of a roll motion feedback
...
\begin{align}
  \dot{\vec{x}} = \left(\mathbf{A} - \left[0 \quad k_{\dot{\phi}} \quad 0 \quad 0\right] \mathbf{B}_\delta\right) \vec{x} + \mathbf{B} \begin{bmatrix} T_{\phi} \\ T_\delta^\textrm{h} \end{bmatrix}
\end{align}
...
such that the eigenvalues of \(\left[\mathbf{A} - \left[0 \quad k_{\dot{\phi}}
\quad 0 \quad 0\right] \mathbf{B}_\delta\right]\) have negative real parts for
\(v_{weave} < v < v_{capsize}\). With gain scheduling with respect to \(v\),
the speed range where the bicycle is stable can be maximized within the limits
of any physical acutator magnitude and bandwidth limits. \citet{Schwab2008}
...
that is riderless stable at a low speed range is beneficial in preventing
whether the rider will fall. To do this, we apply varying magintidue mechanical
perturbations to the handlebars while the rider is balancing the bicycle on a
treadmill and then assess the riders' probability of falling with the balance
assist system on versus off.
