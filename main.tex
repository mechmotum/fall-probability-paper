...
1970s. \Citet{vanZytveld1975} was influenced by Whipple's work and seems to be
...
bicycle with a MEMs rate gyroscope (Company, Location). The balance assist
control algorithm is implemented on a microprocessor (Teensy, Company,
Location) and data from all sensors is logged with a CAN bus (Company,
Location) at approximately X Hz.
...
%
\begin{align}
  T^\textrm{m}_\delta = -k_{\dot{\phi}}\dot{\phi} = k(v_{weave} - v)\dot{\phi}
\end{align}
%
...
Van De Velde's MSc thesis~\citep{vandeVelde2022a} and the physical arrangement
...
(6~\si{\kilo\meter\per\hour}). The 10~\si{\kph} group's experiments occurred X
