...
they are likely to fall, Figure~\ref{fig:balance-assist-bicycle}.
...
bicycle that is stable in a large low speed range (when riderless) assists the
rider in balancing by investigating whether it is beneficial in preventing
falling.  To do this, we apply varying magnitude mechanical perturbations to
the handlebars while the rider is balancing the bicycle on a treadmill and then
...
bicycle with a MEMs rate gyroscope (CRS 03-02s, Silicon Sensing, United Kingdom).
\todo{Do you mean the gyro inside the bicycle or the external? Currently filled in the 
data of the external one. Not sure what make and model is inside the bicycle}
The balance assist control algorithm is implemented on a microprocessor (Teensy, PJRC,
USA) and data from all sensors is logged with a CAN bus (CanEdge2, CSS Electronics,
Denmark) at approximately 100 Hz. \todo{Sample rate is not the same for each
sensor: wheel speed, imu = 1ms, steer torque command is updated every 10ms, 
can log is written every 10ms}
...
250~\si{\newton}. One load cell was damaged during pilot experiments, and is
...
assembly (handlebars, forks, wheel) at magnitudes varying from 20 to 200. The four
motors are arranged at the four corners of a 1~\si{\meter} wide treadmill 
...
\begin{table}
  ...
  \caption{Logistic regression coefficient estimates at
    ...
    frequentist single-level logistic regression.}
...
\begin{figure}
  ...
  \caption{Box plot or distribution plot showing graphical view of stats
  tables.}
\end{figure}
...
odds that a fall occurs while cycling at a speed of 6~\si{\kph}.
Figure~\ref{fig:probability-6kph} visualizes the probability of falling as a
function of the angular impulse while keeping all other explantory factors at
their mean values. This figure is derived from the result that the
balance-assist system halves the odds that a perturbation results in a fall.
...
impulses the probability to fall is zero for both states. But for impulses in
the magnitude region around the mean fall threshold, the probability of falling
is significantly lowered.

but at
10~\si{\kph} that is not the case \((p=0.07)\).  How much this changes the probability that a
