...
they are self-stable~\citep{Astrom2005} but since the 1980s, it has been known
that applying a motor acutated steering torque proportional to the roll rate
...
In the early years of developments automatic control, \citep{Whipple1899}
remarkably realized and showed that roll motion feedback can stabilize the a
bicycle. Much later, automatic roll stabilization of single track vehicles
began in earnest after predictive motorcycle models were developed and refined
in throughout the 1970s. \Citet{vanZytveld1975} was influenced by Whipple's
work and seems to be the first to attempt to robotically stabilize a small
motorbike with a controlled inverted pendulum that mimicked rider lean, but he
was not successful in demonstrating what his theoretic control model correctly
...
early 1980s that \cite{Nagai1983} sucessfully demonstrated balancing a robotic
...
propose how the gains must change with respect to vehicle speed for favorable
...
concerned with wobble instabilities, which are not prominent in bicycles at low
...
up to some limits, of the human controlled plant. In our prior study, we
demonstrate reduced motion during distractions~\cite{Alizadehsaravi2023} due to
the balance assist but conversely researchers with a similar vehicle recently
showed rider disatisfction with the stabilization~\cite{Hanakam2023}.
...
self-stablity. The bicycle model is ideal for showing the effect of a roll
...
up to 7~\si{\newton\meter} of torque between the head tube and steer tube,
Figure~\ref{fig:balance-assist-bicycle}. A custom motor controller converts the
commanded torque to applied torque. We measure the speed of the rear wheel with
an encoder (Magura XXX) and measure the body fixed roll rate of the bicycle
with a MEMs rate gyroscope (Company, Location). The balance assist control
algorithm is implemented on a microprocessor (Teensy, Company, Location) and
data from all sensors is logged with a CAN bus (Company, Location) at
approximately X Hz.
%
\begin{figure}
  \centering
  \includegraphics[width=100mm]{figures/balance-assist-bicycle.jpg}
  \caption{Balance assist bicycle prototype with steer motor and data
    aquisition and control electronics mounted in the rear rack.}
...
microprocessor (Arduino XXX, Italy). We measure the force applied by each motor
at the handlebar via four inline load cells, each with a max load of
...
varying from X to X. The four motors are arranged at the four corners of a
1~\si{\meter} wide treadmill (make, Company, City) that can reach speeds of
18~\si{\kilo\meter\per\hour}. The general design is described in detail in the
Van De Velde's MSc thesis~\citep{vandeVelde2022a} and the physical arrangment
is shown in Figure~\ref{fig:participant-in-set-up}. Our modifications relative
to that described in the thesis include simplifying the controller with a
inexpensive microcontroller and the use of a simpler safety harness.
%
\begin{figure}
  \centering
  % TODO : scale these pngs to a reasonable size for an A4 sheet
  \subcaptionbox{
    A participant on the bicycle in the safety harness with the Bump'Em motor
    lines attached to the ends of the handlebars.
    \label{fig:participant-in-set-up}
  }{
    \includegraphics[width=70mm]{figures/participant-in-set-up.png}
  }
  \subcaptionbox{
    Top view diagram of bicycle handlebar perturbation system.
...
\begin{figure}
  \includegraphics[width=150mm]{figures/perturbation_0.png}
...
(10~\si{\kilo\meter\per\hour}) with the controller gain at $k=8$ and the second
group performed the protocol at a belt speed of 1.7~\si{\meter\per\second}
(6~\si{\kilo\meter\per\hour}). The 10~\si{\kph} group's experiments occured X
...
location of their front wheel on the center line of the treadmill. We define a
...
the treadmill. Figure~\ref{fig:fall} shows an example fall. We then applied
perturbations in random directions (clockwise or counter clockwise) increasing
the magnitude by X~\si{\newton\meter} each time.  We log the magnitude that
causes the first fall to characterize that subject's nominal fall threshold.
Following this we apply 20 perturbations of random magnitudes to the cyclist
while they ride at a constant speed and record which perturbations cause a
fall. We let the cyclist rest and then perform another 20 perturbations. We
randomize whether the balance assist system is on or off during the first or
second set of perturbations.
%
\begin{figure}
  \centering
  \subcaptionbox{Start}{\includegraphics[width=60mm]{figures/start.png}}
  \subcaptionbox{During}{\includegraphics[width=60mm]{figures/during.png}}
  \subcaptionbox{After}{\includegraphics[width=60mm]{figures/after.png}}
  \subcaptionbox{Recovery}{\includegraphics[width=60mm]{figures/recovery.png}}
