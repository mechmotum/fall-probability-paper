...
control, and engagement with a curb, pothole, or bollard are most the common
...
handlebar because of this superposition principle and that it was both
practical and required a low force to cause a fall.
...
Figure~\ref{fig:eigenvalues}. Notably, the bicycle is not stable at 6~\si{\kph}
but the time constant of the unstable eigenvalue is increased by a factor of 8
and at 10~\si{\kph} the system is just stable. Figure~\ref{fig:eigenvalues}
also shows the identified weave mode captured using the methods of
Kooijman~et~al.~\cite{Kooijman2009a} and gives an indication of the performance
of the system, with the model predicting the weave mode well the region of the
tested speeds.
%
\begin{figure}
  \centering
...
two actions which necessitated stopping the treadmill belt and the simulate
...
a full range of perturbation magnitudes, we then choose perturbation forces
...
would be required, but factored this into the later analysis as an alternative.
...
estimate.  The five possible perturbations are separated by 10~\si{\newton}
steps. For example, if the initial estimate of the perturbation threshold of a
participant is 80~\si{\newton}, the five possible perturbations are 60, 70, 80,
90 and 100~\si{\newton}. Which one of these five perturbations is chosen first
is determined at random. If the perturbation results in a fall, the estimate of
...
the model for each participant, the intraclass correlation (ICC) is less that
...
Statistics were computed with R version 4.1.3~\cite{RCoreTeam2021} the lme4
...
human's neuromuscular system~\cite{Magdaleno1971}, but may feel unusual as it
is more akin to what the steering would feel like at in the
\SIrange{30}{40}{\kph} speed range for a normal bicycle. Hanakam, Wehner, and
Wrede~\cite{Hanakam2023} reported dissatisfaction in subjective rider feeling
on their similar bicycle to ours and this high frequency effect to the
human-controlled plant dynamics could be an explanation to their findings.
