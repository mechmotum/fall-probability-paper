...
\todo[inline]{Could add the experimental weave mode identification to this
figure and consider lowering the gain on the model to make the model match the
experimental values.}
%
\begin{figure}
  \centering
...
load cells except the left rear load cell that has a max load of
...
magnitudes varying from \SIrange{16}{160}{\newton\meter}. The four motors
(EC-90 Flat, Maxon Group, Switzerland) are arranged at the four corners of a
1~\si{\meter} wide treadmill that can reach speeds of
18~\si{\kilo\meter\per\hour}. The general base design of this system is
...
the experiments. The subjects were all confident in their cycling skills and
had cycled at least once in the last month. Eleven subjects performed the
experiments at 6~\si{\kilo\meter\per\hour} (1.7~\si{\meter\per\second}) and
fifteen subjects performed the experiments at 10~\si{\kilo\meter\per\hour}
...
foot down onto the treadmill surface or their wheel of the bicycle exits the
...
\begin{table}
  ...
  \caption{Raw measurements taken during each trial. ``Fall outcome'' and
    ...
    remaining measures are time varying.}
  \begin{tabular}{llll}
    \toprule
...
\begin{table}
  ...
  \begin{tabular}{llll}
    ...
    \(\delta_0\) & ind. & \si{\degree} & steer angle at start of perturbation \\
    \(\phi_0\) & ind. & \si{\degree} & roll angle at start of perturbation \\
    \(f\) & dep. & boolean & outcome: did not fall 0 or did fall 1 \\
    \(c\) & ind. & boolean & balance assist: off 0 or on 1 \\
    \(j\) & ind. & integer & order number of perturbation \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}
  \centering
  \caption{Logistic regression coefficient estimates at 10~\kph and gain $g=8$.
    Multiplicative change in odds of predictor variables at 10~\kph based on
...
angular impulse given the estimates in Table~\ref{tab:freq-coefs-6} which
